...
f = open(r"C:\Users\anjal\PycharmProjects\2020nataliapaper\venv\Files\layout_benchmark.txt",'r')
text = f.read()
...
results = open(r"C:\Users\anjal\PycharmProjects\2020nataliapaper\venv\Files\Results_benchmark.txt", "w")
...
x = {(i, j, t): m.addVar(vtype = "B", name="x_" + str((i, j, t))) for i, j in arcs for t in pipe_type}
print (len(x))
print (x)
# Continuous variable representing the flow rate in each pipe
# y = m.addVar(arcs, pipe_type, vtype="C", name="y_")
y = {(i, j, t): m.addVar(vtype = "C", name="y_" + str((i, j, t))) for i, j in arcs for t in pipe_type}
print(len(y)/4)
print(y)
...
for i, j in arcs:
    for t in pipe_type:
        m.addCons((x[i, j, t] * (manholes[i][3])) <= 4 * y[i, j, t])  # the inflow is divided in 4 adjacent manholes

# Upper bound for the flow rate in each pipe
for i, j in arcs:
    for t in pipe_type:
        m.addCons(y[i, j, t] <= M * x[i, j, t])  # M is a BIG number (no upper limit)

# There is only one pipe per section of type t going in a specific direction i,j or j,i
for i, j in arcs:
    m.addCons(quicksum(x[i, j, t] + x[j, i, t] for t in pipe_type) == 1)
...
for i in range(numManholes):
    ...
    constraint_expr = quicksum(x[(i, j, 2)] for (i_, j, k) in x.keys() if i_ == i and k == 2)
    
    # Add the constraint as an expression, not a boolean
    m.addCons(constraint_expr <= 1)
...
for i in range(numManholes):
    if i < numManholes - 1:
        ...
        m.addCons(left_expr <= M * right_expr)
        m.addCons(left_expr >= right_expr)
...
for i in manholes.keys():
    ...
    if manholes[i][3] > 0:
        ...
        inflow_expr = quicksum(y[i, j, 1] for (i_, j) in arcs if i_ == i)
        # Outflow is not used in the constraint, so it’s omitted here

        # Add constraint to the model
        m.addCons(inflow_expr <= manholes[i][3], "inflow_limit")
...
m.addCons(x[upstream, outlet_id, 2] == 1)

# The outflow has to be equal to the sum of all inflows
m.addCons(y[upstream, outlet_id, 2] >= total_flow)
...
if(FO == 1):
    m.setObjective(z, sense="minimize")
    m.optimize()
# elif(FO == 2):
...
sol_status = m.getStatus()

if sol_status == "optimal" or sol_status == "feasible":
    print("Optimal value:", m.getObjVal())
    for (i, j, t) in x:
        if m.getVal(x[(i, j, t)]) > 0:
            print(f"x({i},{j},{t}) = {m.getVal(x[(i, j, t)])}")
    for (i, j, t) in y:
        if m.getVal(y[(i, j, t)]) > 0:
            print(f"y({i},{j},{t}) = {m.getVal(y[(i, j, t)])}")
else:
    print("No optimal solution found. Model status:", sol_status)
